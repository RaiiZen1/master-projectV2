...
from common.evaluate import (
    ...
    evaluate_regression,
    evaluate_fidelity_classification,
    evaluate_fidelity_regression,
)
...
class StudentModelBase(ABC):
    ...
    def evaluate(
        ...
        y_pred: np.ndarray,
        y_true: np.ndarray,
        y_teacher_true: np.ndarray,
        original_task_type: Literal["binary", "regression"],
    ) -> Dict[str, float]:
        """
        ...
        if original_task_type == "binary":
            if self.task_type == "binary":
                ...
                fidelity_metrics = evaluate_fidelity_classification(
                    y_pred, y_teacher_true
                )
                ...
                y_prob = np.column_stack(
                    [1 - y_prob, y_prob]
                )  # Create 2D array with probs for both classes
                ...
                class_fidelity_metrics = evaluate_fidelity_classification(
                    y_prob, y_teacher_prob
                )
                metrics.update(class_fidelity_metrics)
                reg_fidelity_metrics = evaluate_fidelity_regression(
                    y_pred, y_teacher_true
                )
    ...
    def predict(self, X):
        ...
        if self.task_type == "binary":
            ...
            with torch.no_grad():
                ...
                return (
                    logits.cpu().numpy().squeeze(-1)
                )  # Squeeze to remove last dimension if needed
...
def get_student_model(
    ...
    if model_type == "catboost":
        ...
        return TabMStudentModel(
            config=config, task_type=task_type, device=device, hyperparams=hyperparams
        )
    elif model_type == "mlp":
        return MLPStudentModel(
            config=config, task_type=task_type, device=device, hyperparams=hyperparams
        )
    elif model_type == "random_forest":
        return RandomForestStudentModel(
            config=config, task_type=task_type, device=device, hyperparams=hyperparams
        )
    else:
